...
import os, re
from contextlib import contextmanager #this is required for opening multiple files
    ...
    try:
        while True:
            ...
            while not ";" in clInput and not "--" in clInput:
                clInput += raw_input("\n enter a command \n").strip('\r')
            clInput = clInput.split(";")[0]
            inputString = str(clInput)
...
#Printing
            ...
            #If doesnt want all attributes, trim down output
                ...
                for line in lines:
                    ...
                    print " | ".join(out)
    #    print "!Failed to query table " + tableName + " because it does not exist" [THIS IS NO LONGER NEEDED]
    except IndexError:
